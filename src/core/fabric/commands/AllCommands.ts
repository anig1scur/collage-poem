...
export type CreateObjectCommand = Command & {
  ...
  readonly data:
    ...
    | {
        ...
        readonly src: HTMLImageElement | string;
      }
    | {
...
export type ResetCanvasCommand = Command & {
  readonly type: 'reset-canvas';
  readonly data: void;
};

export type ExportCommand = Command & {
...
export type LoadCommand = Command & {
  readonly type: 'load';
  readonly data: {
    format: 'json';
    json: string;
