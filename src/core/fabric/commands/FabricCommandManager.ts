...
export class FabricCommandManager extends MementoCommandManager<AllCommands> {
  constructor(public context: FabricContext) {
    super();
  }

  private getFabricObjectFromOptions(
    ...
    switch (data.objectType) {
      ...
      case 'image':
        return new fabric.Image(data.src, data.options);
      case 'group':
        const objects = data.objectsId.map(
          (id) => this.context.getEditorObjectByIdOrThrow(id).fabricObject,
        );
  ...
  executeCommand(command: AllCommands) {
    ...
    switch (type) {
      ...
      case 'editor-object-data-set-key':
        this.context
          .getEditorObjectByIdOrThrow(data.objectId)
          .data.setKey(data.key, data.value);
        break;
      case 'editor-object-data-clear':
        this.context.getEditorObjectByIdOrThrow(data.objectId).data.clearData();
        break;
      default:
